...
# For Baritone 1.11+ (Minecraft 1.21.11)
...
- `schematica` to build the schematic that is currently open in schematica (legacy command, may not be available in all builds)
...
For example, some clients disable direct chat control. (i.e. anything typed in chat without a prefix will be ignored and sent publicly). **This is a saved setting**, so if you run such a client once, `chatControl` will be off from then on, **even in other clients**.
...
## Why can I do `.goto x z` in some clients but nowhere else?
Some clients add custom commands that aren't from Baritone.
